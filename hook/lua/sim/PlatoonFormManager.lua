...
TheOldPlatoonFormManagerNutCracker = PlatoonFormManager
PlatoonFormManager = Class(TheOldPlatoonFormManagerNutCracker) {
        ...
        if self.Brain.BuilderManagers[self.LocationType] and builder.Priority >= 1 and builder:CheckInstanceCount() then
            ...
            if formIt and builder:GetBuilderStatus() then
                ...
                if self:IsPlattonBuildDelayed(builder.DelayEqualBuildPlattons) then
                    --WARN('*AI NutCracker: ARMY ', repr(self.Brain:GetArmyIndex()),': Platoon Form Manager: IsPlattonBuildDelayed TRUE - ',repr(builder.BuilderName),': Location = ',self.LocationType)
                    return
                end
